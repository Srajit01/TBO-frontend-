...
import { 
  Loader2, MapPin, Clock, Wifi, Utensils, 
  Bed, Car, Image as ImageIcon, X, ZoomIn, ZoomOut,
...
const ImageModal = ({ image, isOpen, onClose, images, activeImageIndex, setActiveImageIndex }) => {
...
const HotelDetailPage = () => {
  ...
  useEffect(() => {
    const fetchHotelDetails = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`http://localhost:9090/api/hotel/${hotelCode}`);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
            ...
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
              {hotel.Images.map((_, index) => (
                <button
                  ...
                  className={`h-2 w-2 rounded-full ${index === activeImageIndex ? 'bg-blue-600' : 'bg-gray-300'}`}
                />
              ))}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            {hotel.Images.slice(0, 4).map((image, index) => (
              <div 
                key={index} 
                className={`rounded-lg overflow-hidden cursor-pointer hover:opacity-75 transition ${index === activeImageIndex ? 'border-2 border-blue-500' : ''}`}
                onClick={() => {
                  setActiveImageIndex(index);
            ...
            {hotel.Images.length > 4 && (
              ...
              >
                <div className="text-center">
                  ...
                  <p className="text-sm text-gray-600">+{hotel.Images.length - 4} More</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <motion.div 
              ...
              variants={fadeVariants}
            >
              <h2 className="text-2xl font-semibold mb-4 border-b pb-3">About the Hotel</h2>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {(showFullFacilities 
                  ...
                  <motion.div
                    ...
                    variants={fadeVariants}
                  >
                    {ICON_MAP[facility] || <MapPin className="mr-2 text-gray-500" />}
          ...
          <div>
            ...
            <motion.div 
              ...
              variants={fadeVariants}
            >
              <h2 className="text-2xl font-semibold mb-4 border-b pb-3">Contact Information</h2>
